...
app = typer.Typer(
    help="kb-wiki - Static wiki compiler and live server for the kb stack"
)
...
def build(
    ...
    dist: Path = typer.Option(
        Path("dist"), help="Path to output static site directory"
    ),
):
...
def serve(
    ...
    dev: bool = typer.Option(
        False, "--dev", help="Enable developer mode (filesystem watcher & auto-rebuild)"
    ),
):
